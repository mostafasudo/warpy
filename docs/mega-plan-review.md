...
Use this document only after a draft implementation plan already exists. First create the plan. Then use this review to verify the plan's scope, failure modes, test coverage, observability, and rollout posture before implementation starts.
...
2. **Every error has a name.** Don't say "handle errors." Name the specific
   ...
   whether it's tested. `except Exception` is a code smell — call it out.
...
* ASCII diagrams in code comments for complex designs — models (state transitions),
  services (pipelines), controllers/endpoints (request flow), stores/hooks (state
  behavior), tests (non-obvious setup).
...
git diff main --stat
git stash list
rg -l "TODO|FIXME|HACK|XXX" --glob "*.py" --glob "*.ts" --glob "*.tsx" --glob "*.js" --glob "*.jsx"
git diff --name-only --diff-filter=ACMRT HEAD~20..HEAD | head -20
```

Then read `AGENTS.md`, `CLAUDE.MD`, `TODOS.md` if it exists, and any relevant docs
or architecture references. Map:
...
```
  ...
  -------------------------|-----------------------------|----------------------------
  AgentExecutionService.run| Upstream API timeout        | httpx.TimeoutException
                           | API returns 429             | httpx.HTTPStatusError
                           | API returns malformed JSON  | json.JSONDecodeError
                           | DB connection pool exhausted| sqlalchemy.exc.TimeoutError
                           | Record not found            | sqlalchemy.exc.NoResultFound
  -------------------------|-----------------------------|----------------------------

  EXCEPTION CLASS               | RESCUED?  | RESCUE ACTION          | USER SEES
  ------------------------------|-----------|------------------------|------------------------------
  httpx.TimeoutException        | Y         | Retry 2x, then raise   | "Service temporarily unavailable"
  httpx.HTTPStatusError         | Y         | Backoff + retry        | Nothing (transparent)
  json.JSONDecodeError          | N ← GAP   | —                      | 500 error ← BAD
  sqlalchemy.exc.TimeoutError   | N ← GAP   | —                      | 500 error ← BAD
  sqlalchemy.exc.NoResultFound  | Y         | Return nil, log warning| "Not found" message
```

**Rules for this section:**
* `except Exception` is ALWAYS a smell. Name the specific exceptions.
* `except Exception as exc` with only a generic log line is insufficient. Log
  the full context: what was being attempted, with what arguments, for what
  user/request.
...
* **Dependency risk.** New Python or npm packages? Security track record?
...
* **Error handling patterns.** (Cross-reference with Section 2 — this section
  ...
  - For every new `except` block: is the error logged with context?
  - Is recovery attempted?
  - Does it leak implementation details to users?
  - Is the rescue too broad? (`except Exception` vs `except SpecificError`)
...
**For LLM/prompt changes:** Check `AGENTS.md`, `CLAUDE.MD`, and
`docs/prompt-engineering.md`. If this plan touches prompt, tool, or agent
instruction files, state which eval suites must be run, which cases should be
added, and what baselines to compare against.
...
* **N+1 queries.** For every new SQLAlchemy relationship traversal: is there
  a `selectinload`/`joinedload` or equivalent? Show the query count in the
  worst case.
...
* **Admin tooling.** New operational tasks (re-running a job, clearing a cache,
  inspecting a queue) that need admin UI, scripts, or operational commands?
...
* **Ecosystem fit.** Does this align with where the FastAPI, SQLAlchemy, React,
  and TypeScript ecosystems are heading?

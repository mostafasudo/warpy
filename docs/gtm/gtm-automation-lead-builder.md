...
- In the Amplemarket UI, open `https://app.amplemarket.com/dashboard/lead_lists`, search or navigate to the exact temporary list, and confirm the visible name plus checkpointed ID when available. On the list row, click the row checkbox, use the floating selection toolbar's `...` menu, choose `Delete`, then confirm the `Delete list` modal. Do not use the lead-list detail page, row sequence actions, or any ambiguous bulk action for deletion.

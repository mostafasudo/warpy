...
- Added two always-available agent tools:
  - `frontend_context`: request a focused DOM snapshot (optional screenshot) only when needed.
...
- Added on-demand screenshot handling and backend sanitization to avoid persisting large data URLs.
...
sequenceDiagram
  ...
  W->>W: Collect DOM subset (+ optional screenshot)
  W->>B: toolResults (frontend_context, consumeAction=false)
  B->>L: ToolMessage + image (if provided)
...
- Filter by visibility and viewport by default (optional overrides).
...
## Screenshot flow (optional)
- On-demand only via `includeScreenshot: true`.
- Captured with html2canvas (viewport only, compressed JPEG).
- Passed back in tool results and attached as a multimodal message to the LLM when present.
- Stored message history is sanitized to strip data URLs from tool messages.
...
- Context is capped and scored to limit payload size.
- Screenshot is opt-in and stored only in live LLM context (sanitized for storage).
...
- Tool results with screenshots are bridged to the LLM as multimodal input and stripped from persistence.
...
- Optional screenshot capture via html2canvas.
...
- Anthropic's computer use tool exposes screenshots plus mouse/keyboard control for desktop automation, suggesting a screenshot-first, low-level interaction approach. Source: Anthropic docs. [3]
- Claude Code focuses on selective context acquisition and execution inside the user's environment (terminal/IDE), highlighting agentic workflows with minimal context switching. Source: Claude Code product page. [4]

## References
1. https://www.perplexity.ai/enterprise/comet
2. https://openai.com/index/introducing-chatgpt-atlas/
3. https://docs.anthropic.com/en/docs/build-with-claude/computer-use
4. https://www.anthropic.com/claude-code/

## Files changed (by area)
Backend:
- `backend/app/services/agent_tools.py` (new frontend tools)
- `backend/app/services/agent_chain.py` (prompt + tool handling + screenshot bridge)

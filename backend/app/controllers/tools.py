...
def read_tools(
    ...
    try:
        items, total = list_tools(session, clerk_session.user_id, page, page_size, search)
        log_info("ToolsController", "read_tools", "Tools fetched", user_id=clerk_session.user_id)
        return PaginatedToolsResponse(items=items, page=page, page_size=page_size, total=total)
    except HTTPException:
        raise
    except Exception as error:
        log_error("ToolsController", "read_tools", "Failed to fetch tools", exc=error, user_id=clerk_session.user_id)
...
def create_tool_route(
    ...
    try:
        tool = create_tool(session, clerk_session.user_id, payload)
        log_info("ToolsController", "create_tool", "Tool created", user_id=clerk_session.user_id)
        return tool
    except HTTPException:
        raise
    except Exception as error:
        log_error("ToolsController", "create_tool", "Failed to create tool", exc=error, user_id=clerk_session.user_id)
...
def replace_tool(
    ...
    try:
        tool = update_tool(session, tool_id, clerk_session.user_id, payload)
        log_info("ToolsController", "replace_tool", "Tool updated", user_id=clerk_session.user_id)
        return tool
    except HTTPException:
        raise
    except Exception as error:
        log_error("ToolsController", "replace_tool", "Failed to update tool", exc=error, user_id=clerk_session.user_id)
...
def remove_tool(
    ...
    try:
        delete_tool(session, tool_id, clerk_session.user_id)
        log_info("ToolsController", "remove_tool", "Tool deleted", user_id=clerk_session.user_id)
    except HTTPException:
        raise
    except Exception as error:
        log_error("ToolsController", "remove_tool", "Failed to delete tool", exc=error, user_id=clerk_session.user_id)

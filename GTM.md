...
Amplemarket-sourced lead identity is trusted GTM data. When a lead's local manifest or Apollo task detail contains the same LinkedIn URL that came from Amplemarket enrichment, the task executor should use that URL as the canonical identity for LinkedIn connection requests. Do not add redundant profile-header, current-company, or page-layout checks that block a blank connection request after the recipient-safety ledger claim has passed. For blank requests, the executor must open the exact trusted profile URL, click the profile `More` menu, and check for the `Connect` action there even when a `Message` button is visible. `Message` is common on LinkedIn profiles and is not proof that the contact is already connected. If `Connect` is available, click it and send the blank request. The only final LinkedIn UI check needed before sending is that the open invitation modal names the intended recipient. If LinkedIn opens a modal for a different person, close it, void the claim, and do not complete Apollo.

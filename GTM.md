...
Amplemarket-sourced lead identity is trusted GTM data. When a lead's local manifest or Apollo task detail contains the same LinkedIn URL that came from Amplemarket enrichment, the task executor should use that URL as the canonical identity for LinkedIn connection requests. Do not add redundant profile-header, current-company, or page-layout checks that block a blank connection request after the recipient-safety ledger claim has passed. For blank connection requests, the only final LinkedIn UI check needed is that the open invitation modal names the intended recipient. If LinkedIn opens a modal for a different person, close it, void the claim, and do not complete Apollo.

...
- Because upstream `gstack` expects a native `.claude/skills/gstack` install, `.claude/skills/gstack`, `.agent/skills/gstack`, and `.cursor/skills/gstack` are symlinks back to `.codex/skills/gstack`, and the top-level skill names `browse`, `careful`, `codex`, `design-consultation`, `design-review`, `document-release`, `freeze`, `gstack-upgrade`, `guard`, `investigate`, `office-hours`, `plan-ceo-review`, `plan-design-review`, `plan-eng-review`, `qa`, `qa-only`, `retro`, `review`, `setup-browser-cookies`, `ship`, and `unfreeze` are mirrored there as symlinks for skill discovery.
- Marketing skills from `coreyhaines31/marketingskills` live under `.codex/skills/marketing/`. In `.claude/skills/`, `.agent/skills/`, and `.cursor/skills/`, expose them only through the single `marketing` symlink/folder. Do not flatten individual marketing skills into the top-level coding skill namespace.
- When a task involves sales, marketing, or GTM, read `GTM.md` first and check the marketing skills pack before choosing an approach.
